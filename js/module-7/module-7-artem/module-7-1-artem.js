...
function onClick(evt) {
  ...
  if (!target.classList.contains('js-target')) {
    return;
    // Если в список классов данного ел. не входит класс "js-target", то мы сразу выходим из нашей функции.
  }
  //   console.log(evt.target);

  // ** Указываем переменную carId (время 2:21:00)
  //  - Мы пытаемся эл. по которому кликнули(target) достать дата-атрибут id (.dataset.carId).
